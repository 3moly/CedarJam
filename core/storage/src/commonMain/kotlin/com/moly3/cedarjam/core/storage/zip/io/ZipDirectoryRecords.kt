    ...
    *
    ...
    open fun encode(buffer: ByteBuffer) {
        buffer.apply {
            ...
            short = name.encodeToByteArray().size.toShort()
...
class ZipDirectoryRecord(
    ...
    override fun allocateBuffer(): ByteBuffer {
        return ByteBuffer(minimumLength + name.encodeToByteArray().size + extra.size + comment.length)
    }
...
class ZipLocalRecord(
    ...
    override fun allocateBuffer(): ByteBuffer {
        return ByteBuffer(minimumLength + name.encodeToByteArray().size + extra.size)
    }
